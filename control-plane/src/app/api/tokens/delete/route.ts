// BFF: Token hard-delete — permanently removes token from DB.
// POST { token, tenant_id, confirm }  confirm must equal "DELETE"
// RBAC: admin only (hard-delete is irreversible)
...
export async function POST(request: NextRequest) {
  ...
  if (confirm !== "DELETE") {
    return NextResponse.json(
      { error: "confirm field must be exactly 'DELETE'" },
      { status: 400 },
    );
  }

  try {
    const res = await fetch(`${TNT_URL}/api/v1/token/delete`, {
      method: "POST",
    ...
    if (!res.ok) {
      return NextResponse.json(
        { error: String(resBody.detail ?? "Delete failed") },
        { status: res.status },
      );
    }
    return NextResponse.json({
