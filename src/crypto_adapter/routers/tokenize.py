...
async def batch_tokenize(
    ...
    outcomes = await asyncio.gather(*(_tokenize_one(item) for item in items), return_exceptions=True)
    ...
    for idx, outcome in enumerate(outcomes):
        if isinstance(outcome, Exception):
            results.append(BatchTokenizeResultItem(index=idx, success=False, error=str(outcome)))
            error_count += 1
        else:
            results.append(BatchTokenizeResultItem(index=idx, token=outcome, success=True))
            success_count += 1

    return BatchTokenizeResponse(results=results, success_count=success_count, error_count=error_count)
